...
async fn main() {
    ...
    let monte_carlo_state = Arc::new(Mutex::new(
        tasks::monte_carlo::MonteCarloState::default()
    ));
    tasks::monte_carlo::setup_monte_carlo_callbacks(&ui, monte_carlo_state);
    log::info!("Monte Carlo module initialized");
